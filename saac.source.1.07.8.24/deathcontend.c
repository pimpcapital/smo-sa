...
int PKLIST_UpdateOnePkTeamData( int ti, int forti, int winerflg)
{
	...
	}else if( PkTeamList[ti].lost > DEFMAXBATTLENUM*DEFLOSERATE){// 0.7 = (1-勝率3成)*最高場次
...
extern gmsv gs[MAXCONNECTION];
	...
	//andy_log
	log( "Update() [%d,%d] %s [%d,%d] ", meti, menum, (winer==0)?"勝":"負", toti, tonum);
...
//		if( find == 0 )continue;
		...
		PkTeamList[i].score = (PkTeamList[i].win*3) - (PkTeamList[i].lost*1);//積分
		if( PkTeamList[i].score >= DEFWINSCORE ){
			PkTeamList[i].inside = 2; //錄取
		}else if( PkTeamList[i].lost > DEFMAXBATTLENUM*DEFLOSERATE){// 0.7 = (1-勝率3成)*最高場次
	...
	while( fgets( line, sizeof( line), fp) ) {
...

		{
			...
			while( fgets( bufarg, sizeof( bufarg), ffp) ) {
				...
				if( bnum >= MAXBATTLENUM )break;//超過戰鬥場次
				for( k=0; k<10; k++){
					easyGetTokenFromBuf( bufarg, '|', k+1, buf1, sizeof( buf1 ));
					if( strlen( buf1) <= 0 ) continue;
					easyGetTokenFromBuf( buf1, ',', 1, buf2, sizeof( buf2 ));
...
/*
			fprintf( fp, "%dth 隊伍:%-32s 勝:%-4d 負:%-4d 場數:%-4d 積分:%-4d\n",
...
int PKLIST_LoadUserfromFile( char *dirpath, char *listfilename )
{
	...
	while( fgets( line, sizeof( line), fp) ) {
		...
		while( 1){
			...
			easyGetTokenFromBuf( line, ',', spoint++, buf1, sizeof( buf1 ));
			if( strlen( buf1) <= 0 ) break;
			sprintf( PkTeamList[count].MyTeamMans[ManNum].cdkey, "%s", buf1); //帳號
...
/*
序號|隊名|帳號_人物名|帳號_人物名|帳號_人物名|帳號_人物名|帳號_人物名|勝|敗|0

#序號|目錄|隊名|帳號_人物名|勝|敗|場|積分|0

1|0x10|第1隊|cdkey|勝|敗|場|積分|0
2|0x10|第2隊|cdkey|勝|敗|場|積分|0
3|0x10|第3隊|cdkey|勝|敗|場|積分|0
4|0x10|第4隊|cdkey|勝|敗|場|積分|0
5|0x10|第5隊|cdkey|勝|敗|場|積分|0
6|0x10|第6隊|cdkey|勝|敗|場|積分|0
7|0x10|第7隊|cdkey|勝|敗|場|積分|0
8|0x10|第8隊|cdkey|勝|敗|場|積分|0
9|0x10|第9隊|cdkey|勝|敗|場|積分|0
10|0x10|第10隊|cdkey|勝|敗|場|積分|0
*/
#endif
