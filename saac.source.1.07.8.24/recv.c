...
if (sasql_query(id,pas)){
...
#ifdef _UNLOCKPOOL // 登入時檢查是否已在UnlockPool中
	if( inUnlockPool(id) ) {
		log(" 已經在UNLOCKPOOL中 ");
    ...
    // Spock 2000/10/31
    log( "charload: added task %s %s, process=%d\n", id,pas,process );
    // Spock end
}
...
/*
   ...
   char *charinfo : 白央奶伙卞忡繡允月平乓仿弁正及旦  □正旦樹  〔
   int id : 丟永本□斥id
*/
#ifdef _NewSave
...
//	log( "unlock:%d, process:%s\n", unlock, process);
    if( unlock) {
		...
		if( addWorkerTask( id , "dummy" , "3" , dummyCallback , ti ,
	        ...
	        log("Game end success, process=%s\n" , process );
	    }
...
#ifdef _DEATH_FAMILY_LOGIN_CHECK //無法刪除人物資料
	saacproto_ACCharDelete_send( ti , FAILED , "server load too high",
							mesgid );
	return;
#endif

#ifdef _DEATH_CONTEND //無法刪除人物資料
	saacproto_ACCharDelete_send( ti , FAILED , "server load too high",
							mesgid );
	return;
#endif
								  ...
								  char *id, int char_num, int date, int backup_flag)
{
	...
	if( backup_flag == 1 ){ // 備份日期
		// 回傳有備份的日期
		get_backup_date(fd, uid, userfdid, GmCliId, id, char_num, date);	

	}else if( backup_flag == 2 ){	// 人物資料
		get_online_player_data(fd, uid, userfdid, GmCliId, id, char_num, date);	// 傳回線上人物資料
		get_backup_player_data(fd, uid, userfdid, GmCliId, id, char_num, date);	// 傳回備份人物資料
	}
...
void saacproto_LoadNEWPlayer_recv( int ti , int charaindex, char* filename)
{
	...
	if( LoadNEWPlayerListAdd( filename) <= 0 ){
		sprintf( Token, "重讀新手名單-錯誤。");
	}else{
		sprintf( Token, "重讀新手名單-完成。");
	}
...
int UNlockM_UnlockPlayer( void)
{
	...
	for( i=0; i<MAXUNlockM; i++)	{
		...
		if( isLocked( UNlockM[i].PlayerId ) ){
			log( "DELAYUNLOCK: %s is still locked err !!\n", UNlockM[i].PlayerId);
		}else{
			log( "DELAYUNLOCK: %s is unlock!!\n", UNlockM[i].PlayerId);
		}
		reset_UNlockMPlayer( i);
		nums++;
	}
	log( "DELAYUNLOCK: Total %d user be Unlock !!\n", nums);
	return nums;
}
...
#ifdef _RACEMAN
//racetype  0:登記 9:刪除 1~5:讀資料 11~55:讀資料
...
{
	//比較目前的排行 如果可以進入排行榜則寫入  並且回傳目前的排行榜
	FILE *fp;
	char rankfile[64];
	char allfile[64];
	char line[512];
	int  i,j,findflag = 0;
	char buf[64];
	char data[4096];  //看server要多少筆就塞多少資料
	...
	if(racetype == 0){
		...
		for(i=0; i < DEFMAXRANK ;i++)	
		{
			...
			if( catchcnt > ranktable[i].catchcnt){   //只有大於才能把人家擠掉  
