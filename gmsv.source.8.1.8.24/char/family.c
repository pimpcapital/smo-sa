...
void ACJoinFM(int fd, int result, int recv)
{
  ...
  if(!CHAR_CHECKINDEX(meindex))	return;
   ...
   if (result == 1) {
   		CHAR_setInt( meindex , CHAR_FMTIMELIMIT, 0 );		
			lssproto_WN_send( fd, WINDOW_MESSAGETYPE_MESSAGE,
			WINDOW_BUTTONTYPE_OK,
			-1, -1,
...
#ifdef _FM_JOINLIMIT
		CHAR_setInt( meindex, CHAR_FMTIMELIMIT, (int)NowTime.tv_sec+(getFMTimeLimit())*(60*60) );
#endif
#ifdef _NEW_MANOR_LAW
		CHAR_setInt(meindex,CHAR_MOMENTUM,0);
...
#ifdef _JZ_BASE_IF
	if ((fmlevel < MINFMLEVLEFORPOINT) || (fmdptop.fmtopdp[iFmIndex]<2000)){//2000家族聲望 or 人數小於30人
#else
  if ((fmlevel < MINFMLEVLEFORPOINT) || (fmdptop.fmtopdp[iFmIndex]<getbaseif())){//CF自定義家族聲望 or 人數小於30人
#endif
	lssproto_WN_send( fd, WINDOW_MESSAGETYPE_MESSAGE,
		WINDOW_BUTTONTYPE_OK,
		-1, -1,
