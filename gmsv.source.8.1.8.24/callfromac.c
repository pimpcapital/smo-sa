...
void saacproto_ACGmsvDownRequest_recv( int fd, int min)
{
	...
	snprintf( buff, sizeof( buff),"石器系統公告。");
...
void saacproto_ACCharLoad_recv(int fd , char* result,char* data,int retfd,int saveindex)
{
    ...
    if(( strcmp( result , SUCCESSFUL ) == 0 )&&(data[0])) {
		...
		CHAR_login( clifd, data, saveindex, 1 );		// 壞玩家
#else		
...
/*
 *  夫永弁及忒蠶〔仇木反荸輪卞仄井銀歹卅中〔蜇箕幻廿銀歹木月  爛反卅中
 */
...
{
	CHAR_talkToCli( charindex, -1, "重讀家族資料OK!",  CHAR_COLORYELLOW);
...
#define floor 141
	...
	for( i=0; i<MAX_SCHEDULE; i++){		
		...
		if( fmpks[fmpks_pos+i].dueltime == hour*100 ){
			...
			fmpks[fmpks_pos+i].max_player = 50;			// 亂鬥生存戰
			fmpks[fmpks_pos+i].win = 0;			
			fmpks[fmpks_pos+i].flag = FMPKS_FLAG_SCHEDULED;
			fmpks[fmpks_pos+i].setting_timeout = 60 * 2;  // 二小時

			sprintf( msg, "%s 對 %s 家族挑戰設定完成", f1, f2 );
...
{	
	...
	if( strcmp( result, "failed" ) == 0 ){
		...
		CHAR_talkToCli( charindex, -1, "您未報名跨星系家族戰", CHAR_COLORYELLOW);
		return;
	}


	SetFMPetVarInit(charindex); // 清除守護獸 Flag
	...
	if( kindflag == FM_FIX_FMRULE ){
		...
		if( ret == 0 ) {
			...
			LogFamilyBankStone(
				...
				(intdata>0)?"FMBank(存款)(家族銀行)":"FMBank(取款)(家族銀行)",
...
void saacproto_ACNEWPlayerList_recv( int fd, char *CdKey, char *UserName, int charaindex, int RunType)
{
	...
	switch( RunType){
	case NEW_ERR: //無法預期的錯誤！
		CHAR_talkToCli( charaindex, -1, "無法預期的錯誤！", CHAR_COLORYELLOW);
		break;
	case NEW_DELOK://新帳號人物刪除完成！
		sprintf( buf,"新帳號人物[%s]確認完成！", CdKey);
		CHAR_talkToCli( charaindex, -1, buf, CHAR_COLORYELLOW);
		if( CHAR_getInt( charaindex, CHAR_GOLD) >= 990000 ){
			sprintf( buf,"帳號人物[%s]持有石幣將超過100萬，取消領取資格！", CdKey);
			CHAR_talkToCli( charaindex, -1, buf, CHAR_COLORYELLOW);
		}else{
			NPC_ActionAddGold( charaindex, 10000);
		}
		break;
	case NEW_DELERR: //新帳號人物刪除發生錯誤！
		CHAR_talkToCli( charaindex, -1, "此帳號人物無法領取石幣！", CHAR_COLORYELLOW);
		break;
	case NEW_ADDOK:
		sprintf( buf,"新帳號人物[%s]增加完成！", CdKey);
		CHAR_talkToCli( charaindex, -1, buf, CHAR_COLORYELLOW);
		break;
	case NEW_ADDERR:
		sprintf( buf,"新帳號人物[%s]增加發生錯誤！", CdKey);
		CHAR_talkToCli( charaindex, -1, buf, CHAR_COLORYELLOW);
		break;
	case NEW_LISTFULL:
		CHAR_talkToCli( charaindex, -1, "新帳號人物列表已滿！", CHAR_COLORYELLOW);
		break;
	}
}
#endif
	...
	case 1:	//跨星球踢人
		{
			...
			for(i=0;i<MAX_USER;i++){
				...
				if(i_use){
					...
					if( !strcmp( cdkey, data) ){
						...
						if( retfd == -1 ){
							print(" 因重複登入 ");
							CHAR_talkToCli( fd_charaindex, -1, "因重複帳號登入而斷線！",
								CHAR_COLORYELLOW);
						}
#ifdef _CHANGEGALAXY
						else if( retfd == -2 ){ // Robin add
							print(" 因準備移民:%d->%s ", fd_charaindex, cdkey);
							CHAR_talkToCli( fd_charaindex, -1, "因準備移民而斷線！",
								CHAR_COLORYELLOW);
						}
#endif
#ifdef _TIMEOUTKICK
						else if( retfd == -3 ){ // Robin add
							print(" 因ＷＧＳ點數不足:%d->%s ", fd_charaindex, cdkey);
							CHAR_talkToCli( fd_charaindex, -1, "您的ＷＧＳ點數不足！",
								CHAR_COLORYELLOW);
						}
#endif
						else{
							print(" 因做壞事 ");
							CHAR_talkToCli(fd_charaindex, -1, "因做壞事而斷線。",
								CHAR_COLORYELLOW);
						}
			...
			if( find == TRUE && retfd != -1 && CHAR_CHECKINDEX( cindex) )	{
				...
				sprintf(buf1,"系統將[%s]逐出伺服器。", /*CHAR_getChar( cindex, CHAR_NAME),*/ szName);
				for( i=0; i<MAX_USER; i++){
					if( CONNECT_getUse(i) )	{
						fd_charaindex = CONNECT_getCharaindex( i);
...
void saacproto_ACMissionTable_recv( int fd, int num, int type, char *data, char* angelinfo)
{
	...
	if( type == 1 ) { // get data list
		...
		if( num == -1) {
			sprintf( msg, " MissionTable已滿!! ");
			print( msg);
			LogAngel( msg);
			return;
		}

		sprintf( msgbuf, " 收到AC回應(add data) num=%d nameinfo=%s ", num, angelinfo);
		print( msgbuf);
		LogAngel( msgbuf);
...
{	
	...
	if(racetype == 0 || racetype == 9 ) return;  //因為登記動作而接收到ac的回傳，所以不用處理

	//或許可以原封不動傳給CLIENT
	
	if(racetype == 1){	//gamemode 1 
		memset(ranktable1,0,sizeof(ranktable1));
		for(i=0;i<DEFMAXRANK*3;i++)  //原則上不會跑完
		{
			if( i % 3 == 0){     
