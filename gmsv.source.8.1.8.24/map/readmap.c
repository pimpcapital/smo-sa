...
BOOL MAP_readBattleMapConfFile( char* filename )
{
    ...
    while( fgets( line , sizeof( line ) , file ) ){
		...
		if( line[0] == '$' ){
			...
			if( iRet < 1 ){
				print( "!!!!!ERROR 無法讀取戰鬥地圖(%s)( line %d )\n", filename, linenum );
			}
			for( i = 0; i < iRet; i ++ ){
				BattleMapNo[i] = iPre[i];
				if( BattleMapNo[i] < 0 ){
					print( "!!!!!ERROR 戰鬥地圖號碼為負值 (%s)( line %d )\n", filename, linenum );
				}
			}
		...
		for( i = iFirst; i <= iLast; i ++ ){
			if( MAP_getImageInt( i, MAP_SETED_BATTLEMAP ) > 0 ){
				print( "!!!!!Warning 相同金錢重複設定了兩次(%s)( line %d )(%d)(%d & %d)\n",
					filename, linenum, i,
