	...
	for( i = 0; ToList[i] != -1; i ++ ){
		...
		case BD_KIND_HP:
		    ...
		    if( ridepet == -1 ) {
				...
				//snprintf( szBuffer, sizeof(szBuffer), "(%s)的耐久力回復(%d)", CHAR_getUseName( toindex ), UpPoint );
...
#if 1	// Robin fix 防封包改Magic  圍
	range = MAGIC_getInt( marray, MAGIC_TARGET );
	//print(" MAGIC_TARGET:%d toNo:%d ", range, toNo);
	if( range == 0 && toNo != attackNo ) { // 只能對自己使用
		print("\n 改封包!MAGIC只能對自己使用!!:%s ", CHAR_getChar( charaindex, CHAR_CDKEY) );
		return FALSE;
	}
	if( range == 1 && toNo >= 20 ) { // 只能對單體使用
		print("\n 改封包!MAGIC只能對單體使用!!:%s ", CHAR_getChar( charaindex, CHAR_CDKEY) );
		return FALSE;
	}
#endif
		...
		for(i=0;i<listidx;i++){
			if(def_is_player[i] != -1){
				...
				while(def_magic_exp_add >= lv_up_exp || def_magic_exp_sub < 0){
					...
					// 如果防守方玩家對此魔法的抗性的相剋魔法的經驗值現在可以降級了
...
// Terry add 2004/12/7
	...
	if(CHAR_getInt(charaindex, CHAR_WHICHTYPE) == CHAR_TYPEPLAYER)
	{
		if(BATTLE_CheckSameSide(charaindex, toNo) == 0) //不同邊
		{
			...
			CHAR_talkToCli(charaindex, -1, "復活光鏡守..不得施予非玩家敵方。", CHAR_COLORYELLOW);
			return;
		}
	}
// end
	...
	for( i = 0 ; i < listidx ; i++ ) {
		...
		if (CHAR_createCharacter( 100354, 777, 45, 45, 0, &icindex, &ioindex, 0) == TRUE){
			...
			if( -1 == petidx || CHAR_getInt(petidx, CHAR_HP) <= 0 ){
				...
				//以下的計算,若人物被打死,血不夠扣的話,會將多出來的傷害再累加至寵物身上
				charahurt_temp = charahurt;
				if( ( charahp -= charahurt ) < 0 )
				{
					charahurt = charahp;
...
#endif
{
	...
	int fire_proficiency = -1, electric_proficiency = -1, ice_proficiency = -1;//熟練度
...
int PROFESSION_MAGIC_GET_ICE_MIRROR_DAMAGE( int attackindex, int defindex, int command, int power )
{
	...
	if( command == BATTLE_COM_S_CONVOLUTE ){	// 迴旋攻擊
		int skill_level = CHAR_GETWORKINT_HIGH( attackindex, CHAR_WORKBATTLECOM3);	
		int hit=0;
