...
#ifdef _PETSKILL_FIREKILL //火線獵殺專用
...
#ifdef _PETSKILL_ACUPUNCTURE
    if( react == BATTLE_MD_ACUPUNCTURE ){ //針刺外皮時會無法正常解除被打方的狀態,所以把index換回來
	    defindex = toindex = BATTLE_No2Index( battleindex, defNo );
        statusDefNo = defNo;
	}
#endif
...
#ifdef _PETSKILL_ACUPUNCTURE
    if( react == BATTLE_MD_ACUPUNCTURE ){ //針刺外皮不會反射特殊狀態,所以要改回來
	    defindex = toindex = BATTLE_No2Index( battleindex, defNo );
        statusDefNo = defNo;
	}
	//注意:之後要不要再改回原本反射.......????
...
#ifdef _WOLF_TAKE_AXE				// WON ADD 抓雙頭狼的限制
	...
	for( i=0; i<MAXCAPTRUEFREE; i++ ){
		...
		for( j = 0; j < CHAR_MAXITEMHAVE; j ++ ){
			...
			if( ITEM_getInt( itemindex, ITEM_ID ) == NeedEnemy[ ti].ItemId[i] ){
				...
				//break;//Change fix 抓到一隻寵只刪除一個道具(最後還是決定全刪)
			}
		}
	}
	return TRUE;
...
int BATTLE_NoAction( int battleindex, int attackNo )
{
	...
	//snprintf( szBuffer, sizeof(szBuffer), "(%s)什麼也不能做。",
	//	CHAR_getUseName( attackindex )
	//);
	//BATTLE_BroadCast( battleindex, szBuffer,
	//	(attackNo >= 10)? CHAR_COLORGRAY : CHAR_COLORPURPLE ) ;
...
int BATTLE_PetIn( int battleindex, int attackNo )
{
...
#endif
#ifdef _PETSKILL_BECOMEFOX // 寵物中媚惑術收回後再放出來時要恢復正常狀態
	...
	& CHAR_BATTLEFLG_NORETURN
		...
		//snprintf( szBuffer, sizeof(szBuffer), "(%s)無法回復成(%s)",
		//	CHAR_getUseName( attackindex ),
	...
	//snprintf( szBuffer, sizeof(szBuffer), "(%s)回復成(%s)",
	//	CHAR_getUseName( attackindex ),
	//	CHAR_getUseName( petindex )
	//);
	//BATTLE_BroadCast( battleindex, szBuffer,
	...
	if( CHAR_getInt( defindex, CHAR_HP ) <= 0 )
		...
		if( CHAR_getWorkInt( attackindex, CHAR_WORKBATTLECOM1 ) == BATTLE_COM_S_ANTINTER 
			&& CHAR_getInt( defindex, CHAR_WHICHTYPE ) == CHAR_TYPEPET ){
				...
				flg |= BCF_ULTIMATE_2;
				CHAR_setWorkInt( defindex, CHAR_WORKULTIMATE, 0 );
				damage=1;//因後面有判斷若 damage<=0 則 skill_type=-1 所以這裡damage設為1
		}
#endif
...
#ifdef _SKILL_TOOTH
	case BATTLE_COM_S_TOOTHCRUSHE:	//  齒術 tooth
		flg |= BCF_TOOTH;
		...
		case BATTLE_COM_S_REGRET:
			{
				...
				if( (PROFESSION_BATTLE_StatusAttackCheck( attackindex, defindex, 12, Success ) == 0 )
					...
					CHAR_setWorkInt( defindex, StatusTbl[12], 2 );//暈眩1回合
					CHAR_setWorkInt( defindex, CHAR_WORKBATTLECOM1, BATTLE_COM_NONE );
...
BOOL BATTLE_S_Temptation(	int battleindex,int attackNo,int defNo,	int skill )
{
	...
	if( CHAR_getWorkInt( defindex , CHAR_WORKFIXAI ) <= atoi(magicarg)){
		//技能攻擊
		CHAR_setWorkInt( defindex, CHAR_WORKTEMPTATION , 1 ); //用來解除打自己方1滴血的限制
		...
		if(iType & 0x00000001){
			...
			for(i1=0;i1<i0 - iObjectNum;i1++){	// 計算剩幾個目標沒打
				for(i2=0;i2<iObjectNum;i2++){			// 攻擊物件計數
					// 若 iToList[i] 為 -1 表示已打完所有的目標
					if(iToList[i] == -1) break;			// 這裡的 i 保留型態 2 打完後的值
					// 設定攻擊目標內容
...
#ifdef _JZ_CAPTURE_FREES
BOOL BATTLE_InitCapFree(char *file )
{
	char	line[256];
	char	token[256];
	int i;
	...
	while(1){		
		...
		if (strstr(line,"=")!=NULL){
			...
			if (strcmp(token,"linebuff")==0){
				...
				Tempfun = NeedEnemy;													//避免內存申請錯誤而刪除舊的cap
				...
				if(NeedEnemy==NULL){
					print("allocateMemory 內存不足\n");
					freeMemory( NeedEnemy );
					NeedEnemy = Tempfun;												//還原cap
					return FALSE;
				}
				NeedEnemyNum_JZ = atoi(token);
				memset(NeedEnemy,0,sizeof( NEEDITEMENEMY ) * atoi(token));
				if(Tempfun!=NULL)freeMemory( Tempfun );				//避免內存洩漏
				continue;
			}
		}
		getStringFromIndexWithDelim(line, ",", 1, token, sizeof(token));
		NeedEnemy[listindex].EnemyId = atoi(token);
		for( i=0; i<MAXCAPTRUEFREE; i++ ){
			if(getStringFromIndexWithDelim(line, ",", i+2, token, sizeof(token))){
				NeedEnemy[listindex].ItemId[i] = atoi(token);
			}else{
				NeedEnemy[listindex].ItemId[i] = -1;
				break;
			}
		}
		listindex++;
