...
{
	char			*functionname;		/*	澀爛白央奶伙卞踏仁楮醒及  蟆 */
...
)
{
	...
	char szMagic[] = "magic";
	char szItem[] = "item";
	int magic = 313,item = 19659;	// magic id預設為火魔法等級一,item index預設為19659
...
#define MAXINSLAY 4
	...
	for( i = 0; i < 15; i ++ ) {
		...
		if( i >= MAXINSLAY )	{
			CHAR_talkToCli( charaindex, -1, "精工每次只能選擇四個物品。", CHAR_COLORYELLOW);
			return FALSE;
		}
		...
		if( !strcmp( buf1, "\0") || !strcmp( buf1, "NULL") )	{
			CHAR_talkToCli( charaindex, -1, "這樣物品不適合精工。", CHAR_COLORYELLOW);
			return FALSE;
		}
		if( strstr( buf1, "INSLAY") != 0 )	{
			if( !ITEM_CHECKINDEX( inslayindex))	{
				...
				CHAR_talkToCli( charaindex, -1, "精工每次只能選擇一個武器或防具。", CHAR_COLORYELLOW);
				return FALSE;
			}
		}
...
#define MAXFIXITEM 2
	...
	for( i = 0; i < 15; i ++ ) {
		...
		if( i >= MAXFIXITEM )	{
			CHAR_talkToCli( charaindex, -1, "修復每次只能選擇兩個物品。", CHAR_COLORYELLOW);
			return FALSE;
		}
		...
		if( (itemtype >= ITEM_FIST && itemtype <= ITEM_AMULET) || 
			...
			if( !ITEM_CHECKINDEX( fixindex))	{
				...
				CHAR_talkToCli( charaindex, -1, "修復每次只能選擇一個武器或防具。", CHAR_COLORYELLOW);
				return FALSE;
			}
		}
...
int PETSKILL_Gyrate( int charaindex, int toNo, int array, char *data )
{
	...
	CHAR_setWorkInt( charaindex, CHAR_WORKBATTLECOM2, toNo ); //攻擊對像  23,24,25,26 為單排
	CHAR_setWorkInt( charaindex, CHAR_WORKBATTLEMODE, BATTLE_CHARMODE_C_OK );
