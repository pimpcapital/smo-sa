...
void dchop( char* src , char* del)
{
    ...
    /*
     * src及  欠井日譬屯化中仁〔1  儂卞勾五 del 允屯化毛伙□皿仄化
     * 域譙允木壬壅允〔公仄化戚及  儂卞中仁〔
     */
...
void pohcd( char* src , char* del)
{
    ...
    /*
     * src及蟆井日譬屯化中仁〔1  儂卞勾五 del 允屯化毛伙□皿仄化
     * 域譙允木壬壅允〔公仄化戚及  儂卞中仁〔
     */
 ...
 * 公木毛手午卞荼瞻〔*dest++ 午井銀中凶中仃升｝瑛寧賜輛卞憤褥互
...
/*------------------------------------------------------------
 * strcat毛兜蟈卞  蜇允月〔贊中及匹｝    卅樺寧毛輪中化銀歹卅中儀〔
 * 婁醒
 *  src     char*       葭卞卅月  儂  
...
/*----------------------------------------
 * makeEscapeString匹綜岳今木凶  儂  毛葭卞  允
 * 婁醒
...
{//ttom this function all change,copy from the second
    ...
    for( i =  0 ; i < index ; i ++ ){
          ...
          */
          ...
          /*
           * 戚及伙□皿及啃卞心勾井勻凶贏今午 delim 及贏今毛簫仄化雲仁
          */
...
/*------------------------------------------------------------
 *   儂   "a,b,c,d" 毛婁醒卞醫  允月〔犯白巧伙玄反 0 卞允月〔
 * 井卅曰及  潰楮醒〔蜇箕反切斤勻午贊中〔
 * 婁醒
 *  src         char*   葭及  儂  
...
int bindlocalhost( int port )
{
  ...
  #ifdef _JZ_EPOLL
    if(!setnonblocking(sfd))return -1;			//吉仔ADD 非阻塞模式
  #endif
    if( getReuseaddr() ) {
		int sendbuff;
	    /* 夫□市伙失玉伊旦及瘋厙迕 */
...
static unsigned char BitTable[] =	/*	申永玄及岈太賜毛菅卞允月  □皮伙	*/
{
...
static unsigned short crctab16[] =	/*	crc｛及煌遙  □皮伙		*/
{
