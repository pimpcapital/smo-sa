...
{	//return FALSE 物品丟在地上 TRUE 不丟在地上(可能是販賣道具)
	int exhibition=-1;
	...
	if( CHAR_getWorkInt( charindex, CHAR_MAPTRADETYPE) != exhibition )	{
		print("\n 不是攤位[%d]主人[%d] ", exhibition, CHAR_getWorkInt( charindex, CHAR_MAPTRADETYPE));
		return TRUE;
	}

	if( MAPTRADE_CHECKMAPFULL( floor, x, y) == TRUE )	{
		CHAR_talkToCli( charindex, -1, "前方已有販賣樣品!", CHAR_COLORYELLOW);
		return TRUE;
	}
	...
	if( ITEM_getChar( itemindex, ITEM_SECRETNAME) != NULL )	{
		if( strstr( ITEM_getChar( itemindex, ITEM_SECRETNAME), "(樣)") == NULL )	{
			sprintf( itemname, "%s(樣)", ITEM_getChar( itemindex, ITEM_SECRETNAME));
		}
	}
	...
	ITEM_setChar( CreateItem, ITEM_EFFECTSTRING, "販賣樣品");//ITEM_EFFECTSTRING
	ITEM_setChar( CreateItem, ITEM_ARGUMENT, "販賣樣品");
...
//=======================
		...
		if( CHAR_getWorkInt( masterindex, CHAR_WORKTRADEMODE) == CHAR_TRADE_TRADING ||
			...
			CHAR_talkToCli( charindex, -1, "主人正在買賣中!", CHAR_COLORYELLOW);
			return TRUE;
		}
   		CHAR_talkToCli(charindex, -1, "請稍候。", CHAR_COLORYELLOW);
...
{	//TRADE_DATAS
	...
	if( TRADE_DATAS[ num].masterindex != -1 )	{
		print("\n 刪除攤位[%d], 主人[%d]存在!!", num, TRADE_DATAS[ num].masterindex);
		return;
	}
...
BOOL MAPTRADE_CHECKMASTERIN( int masterindex , int toindex, int num)
{
	...
	if( CHAR_getInt( masterindex, CHAR_FLOOR) != TRADEMAP )	{	//主人不在了
		...
		if( (mapnum == -1 ) ||
			...
			CHAR_talkToCli( toindex, -1, "主人不在攤位上!", CHAR_COLORYELLOW);
			//主人不在位置上
			return FALSE;
		}
...
BOOL MAP_TRADEPETDROP( int charindex, int petindex,int floor, int x, int y)
{
	...
	if( CHAR_getChar( petindex, CHAR_NAME) != NULL )	{
		if( strstr( CHAR_getChar( petindex, CHAR_NAME), "(樣)") == NULL )	{
			sprintf( petname ,"%s(樣)", CHAR_getChar( petindex, CHAR_NAME) );
		}
	}

	//CREAT 販賣樣品	ENEMY_TEST_createPetIndex
	...
	sprintf( TRADE_DATAS[exhibition-TRADESTARTNUM ].Goodname,"寵物-%s", petname);
	return TRUE;
}
