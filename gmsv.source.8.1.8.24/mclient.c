...
#ifdef _GM_WARP_PLAYER				// WON 傳送玩家
...
#ifdef _RECAL_SERVER_OFF			// WON MSERVER 關閉 GS	
...
void mproto_NoTalk_Recv(int fd,char* uid,int userfdid,int nTime)
{
	...
	CHAR_talkToCli( fd_charaindex, -1, "由於搗亂，被GM禁言10到30分鐘", CHAR_COLORYELLOW);
...
void mproto_Jail_Recv(int fd,char* uid,int userfdid)
{
	...
	if (!CHAR_CHECKINDEX(fd_charaindex))
		return;
	CHAR_talkToCli( fd_charaindex, -1, "由於搗亂，被GM關入監牢", CHAR_COLORYELLOW);
...
void mproto_WaeiKick_Recv(int fd,char* uid,int userfdid)
{
	...
	if (!CHAR_CHECKINDEX(fd_charaindex))
		return;
	CHAR_talkToCli( fd_charaindex, -1, "由於搗亂，被GM踢除", CHAR_COLORYELLOW);
...
void mproto_Que_Send( int fd, int uid, int index, char* question)
{
	...
	if( strlen( question) <= 0 ){
		CHAR_talkToCli( index, -1, "字串長度不足！", CHAR_COLORYELLOW);
		return;
	}
...
void gserver_runtime()
{
	...
	checksum += util_mkint(buffer, (int)difftime(starttime1,gserver_runtime_starttime0));//傳送的單位為秒
	util_mkint(buffer, checksum);
