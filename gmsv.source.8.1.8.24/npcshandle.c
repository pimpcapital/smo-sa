				...
				char *CheckfreeMess)
{
	...
	if( Enable == FALSE ){//┮Τ兵ンぃΘミ
		NPCS_NpcstalkToCli( charaindex, npcobjindex, "兵ンぃì", CHAR_COLORYELLOW);
		return FALSE;
	}
	if( ReCall == TRUE ){
		//耞琌recall
...
BOOL NPCS_RunDoEventAction( int charaindex, char *buf1)
{
	...
	if( NPC_Util_GetStrFromStrWithDelim( buf1, "DelItem", buf2, sizeof( buf2)) != NULL ) {
		...
		if( NPC_ActionDelItem( charaindex, buf2) == FALSE ) {
			//print("\n э??NPC_ActionDelItem:%s:%s  ", CHAR_getChar(toindex, CHAR_CDKEY), buf2);
			return FALSE;
		}
	}
